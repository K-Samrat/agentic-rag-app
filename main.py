...
async def stream_and_save(conversation_id: int, question: str, session_id: str, db: AsyncSession):
    # First, verify this session owns the conversation
    ...
    # Get the current title and message count safely
    result = await db.execute(
        select(models.Conversation.title, func.count(models.Message.id))
        .join(models.Message)
    ...
    # Finally, save the assistant's complete message
